...
{
    public class CventEventController : ApiController
    {
        ...
        /*
        public HttpResponseMessage Post(JObject result) 
        {
            string headerValues;
            var BodyData="";
            var MemberCustomFieldGUID = System.Configuration.ConfigurationManager.AppSettings.Get("Cvent.Webhooks.MemberCustomFieldGUID");
            try
            {
                BodyData = Newtonsoft.Json.JsonConvert.SerializeObject(result);
            }
            catch 
            {
                BodyData = "Error converting from  object";
            ...
            bool LogSuccess = WL.LogJsonData(BodyData, headerValues, "POST", true);
                       
            if (IsValidAuth(headerValues))
            {
                if (LogSuccess == true)
                {
                     return Request.CreateResponse<object>(System.Net.HttpStatusCode.Created, result);
                }
                else
                {
                    return new HttpResponseMessage(HttpStatusCode.BadRequest);
